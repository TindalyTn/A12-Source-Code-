...
internal class MobileDevice
{
	...
	{
		IntPtr dirent = IntPtr.Zero;
		int num = AFCDirectoryRead(conn, dir, ref dirent);
		if (num == 0 && dirent != IntPtr.Zero)
		{
			int i = 0;
			List<byte> list = new List<byte>();
			for (; Marshal.ReadByte(dirent, i) > 0; i++)
			{
				list.Add(Marshal.ReadByte(dirent, i));
			}
	...
	public static object AMDeviceCopyValue(IntPtr device, string domain, string name)
	{
		...
		IntPtr srcRef = AMDeviceCopyValue_Int(device, intPtr, intPtr2);
		...
		{
			result = RuntimeHelpers.GetObjectValue(CoreFoundation.ManagedTypeFromCFType(ref srcRef));
