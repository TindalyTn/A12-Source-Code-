...
public class iOSDeviceManager
{
	...
	private void NotificationCallback(ref AMDeviceNotificationCallbackInfo callback)
	{
		try
		{
			...
			case ConnectNotificationMessage.Connected:
			{
				...
				if (iOSDevice5 == null)
				{
					iOSDevice5 = new iOSDevice(callback.DevicePtr);
					currentConnectedDevice.Add(iOSDevice5);
				}
				this.CommonConnectEvent?.Invoke(this, new DeviceCommonConnectEventArgs(iOSDevice5, ConnectNotificationMessage.Connected));
				break;
			}
			case ConnectNotificationMessage.Disconnected:
			{
				...
				if (iOSDevice4 != null)
				{
					currentConnectedDevice.Remove(iOSDevice4);
				}
				this.CommonConnectEvent?.Invoke(this, new DeviceCommonConnectEventArgs(iOSDevice4, ConnectNotificationMessage.Disconnected));
				break;
			}
			}
		}
	...
	{
		return currentConnectedDevice.FirstOrDefault((iOSDevice p) => p.DevicePtr == devicePtr);
	}
	...
	public void StartListen()
	{
		...
		try
		{
			...
			IntPtr am_device_notification_ptr = IntPtr.Zero;
			kAMDError kAMDError = (kAMDError)MobileDevice.AMDeviceNotificationSubscribe(deviceNotificationCallback, 0u, 1u, 0u, ref am_device_notification_ptr);
			...
			IntPtr user_info = IntPtr.Zero;
			kAMDError = (kAMDError)MobileDevice.AMRestoreRegisterForDeviceNotifications(deviceDFUConnectedNotificationCallback, deviceRecoveryConnectedNotificationCallback, deviceDFUDisConnectedNotificationCallback, deviceRecoveryDisConnectedNotificationCallback, 0u, ref user_info);
			if (kAMDError > kAMDError.kAMDSuccess)
			{
